...
class MagicReader {
    ...
    // 🔥 核心修复：使用requestAnimationFrame强制设置时间
    jumpToTime(targetTime) {
        console.log('═══════════════════════════════════');
        console.log('🎯 跳转到:', targetTime, '秒');
        console.log('📍 当前时间:', this.audio.currentTime.toFixed(2), '秒');
        
        // 暂停
        this.audio.pause();
        console.log('⏸️ 已暂停');
        
        // 使用 requestAnimationFrame 确保在下一帧设置时间
        requestAnimationFrame(() => {
            console.log('🔧 Frame 1: 设置时间');
            this.audio.currentTime = targetTime;
            
            requestAnimationFrame(() => {
                console.log('🔧 Frame 2: 再次确认时间 =', this.audio.currentTime.toFixed(2));
                
                // 如果时间不对，再设置一次
                if (Math.abs(this.audio.currentTime - targetTime) > 1) {
                    console.warn('⚠️ 时间不对，重新设置');
                    this.audio.currentTime = targetTime;
                }
                
                requestAnimationFrame(() => {
                    console.log('🔧 Frame 3: 最终时间 =', this.audio.currentTime.toFixed(2));
